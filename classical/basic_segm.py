...
data_rgb = np.squeeze(np.reshape(im_preproc, (1024 * 1024, -1, 3)))
...
dataset_basic = np.zeros((1024 * 1024 * number_images, dim), dtype=int)
mask_vect = np.zeros((1024*1024*number_images, 1), dtype=int)
global_counter = 0
print(dataset_basic.shape)
    ...
    for image in os.listdir(folder_dir_2):
        ...
        for x in range(np.size(im_m, 0)):
            for y in range(np.size(im_m, 1)):
                if im_m[x,y] != 0:
                    dataset_basic[global_counter, 0] = im_preproc[x, y, 0]
                    dataset_basic[global_counter, 1] = im_preproc[x, y, 1]
                    dataset_basic[global_counter, 2] = im_preproc[x, y, 2]

                    mask_vect[global_counter, 0] = im_m[x,y]
                    global_counter += 1
print("dataset shape: ", dataset_basic.shape)
print("mask_vect shape: ", mask_vect.shape)
np.save('./saved/dataset_rgb_basic', dataset_basic)
np.save('./saved/mask_vect_rgb_basic', mask_vect)
