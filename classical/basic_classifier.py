...
data_matrix = np.load('./saved/dataset_rgb_basic.npy')
target_vect = np.ravel(np.load('./saved/mask_vect_rgb_basic.npy'))
...
im_preproc = np.copy(im_i)
im_preproc[:, :, 0] = local_binary_pattern(im_i[:, :, 0], n_points, radius, METHOD)
im_preproc[:, :, 1] = local_binary_pattern(im_i[:, :, 1], n_points, radius, METHOD)
im_preproc[:, :, 2] = local_binary_pattern(im_i[:, :, 2], n_points, radius, METHOD)
...
dim = 3
number_images = 16
dataset_test = np.zeros((1024 * 1024 * number_images, dim), dtype=int)
    ...
    for image in sorted(os.listdir(folder_dir_2), key=len):
        ...
        for x in range(np.size(im_m, 0)):
            for y in range(np.size(im_m, 1)):
                if im_m[x,y] != 0:
                    dataset_test[global_counter, 0] = im_preproc[x, y, 0]
                    dataset_test[global_counter, 1] = im_preproc[x, y, 1]
                    dataset_test[global_counter, 2] = im_preproc[x, y, 2]
...
acc = np.sum(y_pred==target_vect) / np.size(y_pred)
...
for x in range(np.size(im_m, 0)):
    for y in range(np.size(im_m, 1)):
        if im_m[x, y] != 0:
            test_data[global_counter, 0] = im_i[x, y, 0]
            test_data[global_counter, 1] = im_i[x, y, 1]
            test_data[global_counter, 2] = im_i[x, y, 2]
...
print("accuracy: ", np.sum(test_target == y_pred)/np.size(y_pred))
...
plot_two_with_map(im_m, y_pred_img, im_i=im_i, title="Predikce metody RGB - GNB",
                  save_name="./figures/RGB_GNB_1024_correct_3"
                  )
...
plot_two_with_map(im_m[start_x:dest_x, start_y:dest_y], y_pred_img[start_x:dest_x, start_y:dest_y], im_i=im_i[start_x:dest_x, start_y:dest_y],
                  ...
                  save_name="./figures/RGB_GNB_128_correct_3"
                  )
