...
# criterion_dice = dice_loss()  #new, does not function
# criterion_bce = bce_loss()  #new, does not function
...
for epoch in range(num_epochs_to_train):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        loss = criterion_ce(outputs, target)# + alfa*bce_loss(mask, target) + beta*dice_loss(mask, target)  # new
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(eval_loader, 0):
            inputs, target = data#['image'], data['mask']
            inputs, target = inputs.to(device), target.to(device)  # jen pro cuda
            ...
            running_iou += iou(outputs.cpu, target.cpu)
    ...
    if avg_val_accuracy[epoch] > best_val_acc:
        ...
        torch.save(best_model_weights_acc, './models3/model_' + model_num + '_maxi_512_' + str(
            num_epochs_to_train + saved_model_epochs) + '_adam_eval_bestAcc_interrupted.pth')
    if mean_iou_val[epoch] > best_val_miou:
        ...
        torch.save(best_model_weights_miou, './models3/model_' + model_num + '_maxi_512_' + str(
            num_epochs_to_train + saved_model_epochs) + '_adam_eval_bestMiou_interrupted.pth')
    if avg_val_loss[epoch] < best_val_loss:
        best_epoch_loss = epoch
        best_val_loss = avg_val_loss[epoch]
        best_model_weights_loss = model.state_dict()
        torch.save(best_model_weights_loss, './models3/model_' + model_num + '_maxi_512_' + str(
            num_epochs_to_train + saved_model_epochs) + '_adam_eval_bestLoss_interrupted.pth')
    ...
    #             num_epochs_to_train + saved_model_epochs) + '_adam_eval_bestAvgAcc.pth')
    #
    #     if mean_iou_val[epoch-1] > mean_iou_val[epoch] and best_IoU:
    #         torch.save(model.state_dict(), './models/model_' + model_num + '_maxi_512_' + str(
    #             num_epochs_to_train + saved_model_epochs) + '_adam_eval_bestMeanIoU.pth')
    np.save("./metrics3/avg_val_acc_" + model_num + ".npy", avg_val_accuracy)
    np.save("./metrics3/avg_train_acc_" + model_num + ".npy", avg_train_accuracy)
    np.save("./metrics3/avg_val_loss_" + model_num + ".npy", avg_val_loss)
    np.save("./metrics3/avg_train_loss_" + model_num + ".npy", avg_train_loss)
    np.save("./metrics3/avg_val_f1_" + model_num + ".npy", avg_f1_val)
    np.save("./metrics3/mean_val_IoU_" + model_num + ".npy", mean_iou_val)

    torch.save(model.state_dict(), './models3/model_'+model_num+'_maxi_512_' + str(num_epochs_to_train + saved_model_epochs) + '_adam_eval_interrupted.pth')

print('Finished Training')
torch.save(best_model_weights_acc, './models3/model_' + model_num + '_maxi_512_' + str(num_epochs_to_train + saved_model_epochs) + '_adam_eval_bestAcc_'+str(best_epoch_acc)+'.pth')
torch.save(best_model_weights_miou, './models3/model_' + model_num + '_maxi_512_' + str(num_epochs_to_train + saved_model_epochs) + '_adam_eval_bestMiou_'+str(best_epoch_miou)+'.pth')
torch.save(best_model_weights_loss, './models3/model_' + model_num + '_maxi_512_' + str(num_epochs_to_train + saved_model_epochs) + '_adam_eval_bestLoss_'+str(best_epoch_loss)+'.pth')
torch.save(model.state_dict(), './models3/model_' + model_num + '_maxi_512_' + str(num_epochs_to_train + saved_model_epochs) + '_adam_eval_maxEpochs.pth')
print("models saved")
