...
optimizer = optim.SGD(model.parameters(), lr=0.01, momentum=0.9)  #předtím
...
scheduler_multi = MultiStepLR(optimizer, milestones=[4, 6, 8, 10, 12, 15], gamma=0.5)
...
for epoch in range(num_epochs_to_train):
    ...
    with torch.no_grad():
        ...
        print(mean_iou_per_class_val[epoch, :])
        torch.cuda.empty_cache()
    ...
    #
    # torch.save(model.state_dict(), './models_all/'+models_dir+'/model_'+model_num+'_maxi_512_' + str(num_epochs_to_train + saved_model_epochs) + '_SGD_eval_interrupted.pth')
