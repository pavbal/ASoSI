...
optimizer = optim.SGD(model.parameters(), lr=0.005, momentum=0.9)  #předtím
...
# scheduler_multi = MultiStepLR(optimizer, milestones=[2, 4, 8, 16, 24, 27], gamma=0.5)
scheduler_multi = MultiStepLR(optimizer, milestones=[3, 4, 15], gamma=0.5)

scheduler_cond = MultiStepLR(optimizer, milestones=[1], gamma=0.5)
tresh = 0.26
run_sched = False
...
mean_iou_per_class_val = np.zeros((num_epochs_to_train, num_classes-1))
...
for epoch in range(num_epochs_to_train):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(eval_loader, 0):
            ...
            # IoU.update(pred.cpu(), target.cpu())  #  changed, uncomment
        ...
        mean_iou_per_class_val[epoch, :] = np.reshape(np.divide(running_inters, running_unions), (1, num_classes-1))
        mean_iou_val[epoch] = np.mean(mean_iou_per_class_val[epoch, :])
        print('[%d] eval_loss: %.3f\t eval_accuracy: %.3f\t eval_mIoU: %.3f' % (epoch + 1 + saved_model_epochs, avg_val_loss[epoch], avg_val_accuracy[epoch], mean_iou_val[epoch]))
        torch.cuda.empty_cache()
    ...
    if avg_val_accuracy[epoch] > best_val_acc:
        ...
        torch.save(best_model_weights_acc, './models_all/'+models_dir+'/model_' + model_num + '_maxi_512_' + str(
    ...
    if mean_iou_val[epoch] > best_val_miou:
        ...
        torch.save(best_model_weights_miou, './models_all/'+models_dir+'/model_' + model_num + '_maxi_512_' + str(
            num_epochs_to_train + saved_model_epochs) + '_SGD_Zval_bestMiou_interrupted.pth')
        string = "_after_miou"
        # mang = False

    if mang:
        torch.save(model.state_dict(), './models_all/' + models_dir + '/model_' + model_num + '_maxi_512_' + str(
            num_epochs_to_train + saved_model_epochs) + '_SGD_eval_epoch_'+str(epoch)+'.pth')
    ...
    if avg_val_loss[epoch] < best_val_loss:
        ...
        torch.save(best_model_weights_loss, './models_all/'+models_dir+'/model_' + model_num + '_maxi_512_' + str(
            num_epochs_to_train + saved_model_epochs) + '_SGD_eval_bestLoss_interrupted.pth')

    if epoch == 10:
        torch.save(best_model_weights_loss, './models_all/' + models_dir + '/model_' + model_num + '_maxi_512_' + str(
            num_epochs_to_train + saved_model_epochs) + '_SGD_eval_savedAt15epochs.pth')
    ...
    np.save("./models_all/"+metrics_dir+"/avg_val_acc_" + model_num + ".npy", avg_val_accuracy)
    np.save("./models_all/"+metrics_dir+"/avg_train_acc_" + model_num + ".npy", avg_train_accuracy)
    np.save("./models_all/"+metrics_dir+"/avg_val_loss_" + model_num + ".npy", avg_val_loss)
    np.save("./models_all/"+metrics_dir+"/avg_train_loss_" + model_num + ".npy", avg_train_loss)
    np.save("./models_all/"+metrics_dir+"/avg_val_f1_" + model_num + ".npy", avg_f1_val)
    np.save("./models_all/"+metrics_dir+"/mean_val_IoU_" + model_num + ".npy", mean_iou_val)
    np.save("./models_all/" + metrics_dir + "/mean_val_IoU_per_class" + model_num + ".npy", mean_iou_per_class_val)

    torch.save(model.state_dict(), './models_all/'+models_dir+'/model_'+model_num+'_maxi_512_' + str(num_epochs_to_train + saved_model_epochs) + '_SGD_eval_interrupted'+string+'.pth')

print('Finished Training')
torch.save(best_model_weights_acc, './models_all/'+models_dir+'/model_' + model_num + '_maxi_512_' + str(num_epochs_to_train + saved_model_epochs) + '_SGD_eval_bestAcc_'+str(best_epoch_acc)+'.pth')
torch.save(best_model_weights_miou, './models_all/'+models_dir+'/model_' + model_num + '_maxi_512_' + str(num_epochs_to_train + saved_model_epochs) + '_SGD_eval_bestMiou_'+str(best_epoch_miou)+'.pth')
torch.save(best_model_weights_loss, './models_all/'+models_dir+'/model_' + model_num + '_maxi_512_' + str(num_epochs_to_train + saved_model_epochs) + '_SGD_eval_bestLoss_'+str(best_epoch_loss)+'.pth')
torch.save(model.state_dict(), './models_all/'+models_dir+'/model_' + model_num + '_maxi_512_' + str(num_epochs_to_train + saved_model_epochs) + '_SGD_eval_maxEpochs.pth')
print("models saved")
